...
cte_kardex AS (
    SELECT
        ...
        CASE WHEN pa.concepto_pago_id IN (133, 2)
        ...
        CASE WHEN (pa.monto - pa.descuento_total - SUM(IFNULL(dpi.monto, 0))) > 0
                  ...
                  AND pa.concepto_pago_id IN (133, 2)
             THEN 1 ELSE 0
        ...
        MAX(CASE WHEN pa.concepto_pago_id = 133 THEN dpi.fecha_registro_pago END) AS fecha_ultimo_pago_matricula,
...
SELECT
    ...
    /* Matrícula */
    IFNULL(SUM(CASE WHEN kardex.concepto_pago_id = 133 THEN kardex.pagado ELSE 0 END), 0) AS Pago_Matricula,
    IFNULL(SUM(CASE WHEN kardex.concepto_pago_id = 133 AND kardex.monto_descuento != 0 THEN kardex.monto_descuento ELSE 0 END), 0) AS Descuento_Matricula,
    IFNULL(SUM(CASE WHEN kardex.concepto_pago_id = 133 AND kardex.monto_regularizado != 0 THEN kardex.monto_regularizado ELSE 0 END), 0) AS Regularizado_Matricula,
    IFNULL(SUM(CASE WHEN kardex.concepto_pago_id = 133 AND kardex.monto_compensacion != 0 THEN kardex.monto_compensacion ELSE 0 END), 0) AS Compensacion_Matricula,
    IFNULL(SUM(CASE WHEN kardex.concepto_pago_id = 133 AND kardex.monto_liquidado != 0 THEN kardex.monto_liquidado ELSE 0 END), 0) AS Liquidado_Matricula,
    ...
    IFNULL(SUM(CASE WHEN kardex.concepto_pago_id IN (133,2)
    ...
    IFNULL(CASE
        WHEN MAX(det_pag.concepto_pago_id) = 133 THEN 'Matricula'
...
WHERE ei.id IN (0, 1, 2, 3, 4, 5)
  ...
  AND (p.iduniversidad IN (49) OR p.iduniversidad IS NULL)
GROUP BY i.id